...
const ViewAccount = ({ route, navigation }) => {
    ...
    const GetAccount = async () => {
        const response = await fetch(
            ...
            'http://192.168.11.104:3000/NguoiDung/DanhSach?inputID=' + infoAcc._id,
        );
    ...
    const GetFollow = async () => {
        try {
            ...
            const response = await fetch(
                ...
                'http://192.168.11.104:3000/NguoiDung/TheoDoi/DanhSach?idAccount=' + infoAcc._id + '&&idSelf=' + loginId,
            );
            const json = await response.json();
    ...
    const SetFollow = async (is) => {
        try {
            ...
            const response = await fetch(
                // 'https://backend-munnect.herokuapp.com/NguoiDung/DanhSach?inputID='+loginId,
                'http://192.168.11.104:3000/NguoiDung/TheoDoi/TheoDoiMoi?idSelf=' + loginId + '&&idAccount=' + infoAcc._id + '&&isFollow=' + is,
            );
            const json = await response.json();
            console.log(json);
            if (json != undefined) {
