        ...
        try {
            if (route.params.title == "Đang theo dõi") {
                const response = await fetch(
                    ...
                    'http://192.168.11.104:3000/NguoiDung/TheoDoi/DanhSach?idSelf=' + infoAcc._id + "&&getFollowing=true",
                );
                ...
                const response = await fetch(
                    ...
                    'http://192.168.11.104:3000/NguoiDung/TheoDoi/DanhSach?idSelf=' + infoAcc._id + "&&getFollower=true",
                );
                const json = await response.json();
                setarr_follow(json.data.listTheoDoi);
                settype('follower');
