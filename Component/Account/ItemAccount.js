...
const ItemAccount = (row) => {
    ...
    const GetFollow = async () => {
        try {
            ...
            const response = await fetch(
                ...
                'http://192.168.191.8:3000/NguoiDung/TheoDoi/DanhSach?idAccount=' + nguoiDung._id + '&&idSelf=' + loginId,
            );
    ...
    const SetFollow = async (is) => {
        try {
            ...
            const response = await fetch(
                // 'https://backend-munnect.herokuapp.com/NguoiDung/DanhSach?inputID='+loginId,
                'http://192.168.191.8:3000/NguoiDung/TheoDoi/TheoDoiMoi?idSelf=' + loginId + '&&idAccount=' + nguoiDung._id + '&&isFollow=' + is,
            );
            const json = await response.json();
            console.log(json);
            if (json != undefined) {
